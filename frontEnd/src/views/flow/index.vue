            ...
            <avue-crud ref="crud" v-model="form" :option="tableOption" :page.sync="page" :data="list" :search.sync="search"
                ...
                <template slot="menu" slot-scope="{row,index}">
                    ...
                    <template v-if="['0','3'].includes(row.status)">
                        <div>
                            <el-button v-if="row.status!='3'" type="text" size="small" icon="el-icon-check" @click="handleSubmit(row)">提交审核</el-button>
                            <el-button type="text" size="small" icon="el-icon-edit"
                                @click="handleControl('edit', row, index)">编辑</el-button>
                            <el-button type="text" size="small" icon="el-icon-delete"
                                @click="handleControl('del', row, index)">删除</el-button>
                        </div>
                    </template>
                    <template v-if="row.status == 1">
                        <el-button type="text" size="small" icon="el-icon-close" @click="handleSubmit(row)">撤销提交</el-button>
                    </template>
                </template>
...
<script>
import { addObj, updateObj, delObj, getObj, submitObj, getPage } from '@/api/flowMgr/flow'
import { tableOption } from '@/crud/flowMgr/flow'
export default {
    name: 'MenuList',
    components: {},
    data() {
