    ...
    <div style="padding-top:15px;" class="u-crud">
        <avue-crud ref="crud" :page.sync="page" v-model="form" :option="tableOption" :data="list" :search.sync="search"
            ...
            <template slot="finger" slot-scope="{row}">
                <div v-for="item in row.finger" style="text-align: left;">{{ item.name }}</div>
            </template>
...
<script>
import { getPage, exportObj, addObj_tag, delObj_tag ,delObj} from '@/api/asset/site'
import { tableOption } from '@/crud/task/child/site'
export default {
    name: 'MenuList',
    components: {},
    props:{
