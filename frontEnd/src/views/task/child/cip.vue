<template>
    <div style="padding-top:15px;" class="u-crud">
        <avue-crud ref="crud" :page.sync="page" v-model="form" :option="tableOption" :data="list" :search.sync="search"
            ...
            <template slot="ip_count" slot-scope="{row}">
                <a @click="handleJump('ip')">{{ row.ip_count }}</a>
            </template>
            <template slot="domain_count" slot-scope="{row}">
                <a @click="handleJump('domain')">{{ row.domain_count }}</a>
            </template>
            <template slot="menuLeft">
                <el-button size="mini" :disabled="!selectedRows.length" icon="el-icon-delete"
...
<script>
import { getPage, delObj } from '@/api/task/cip'
import { tableOption } from '@/crud/task/child/cip'
export default {
    name: 'MenuList',
    components: {},
    props: {
