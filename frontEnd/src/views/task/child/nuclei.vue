<template>
    <div style="padding-top:15px;" class="u-crud">
        <avue-crud ref="crud" :page.sync="page" v-model="form" :option="tableOption" :data="list" :search.sync="search"
            ...
            <template slot="template_id" slot-scope="{row}">
                <a target="_blank" :href="'https://github.com/projectdiscovery/nuclei-templates/blob/main/http/cves/2021/'+row.template_id+'.yaml'">{{ row.template_id }}</a>
            </template>
...
<script>
import { getPage,delObj} from '@/api/task/nuclei'
import { tableOption } from '@/crud/task/child/nuclei'
export default {
    name: 'MenuList',
    components: {},
    props:{
