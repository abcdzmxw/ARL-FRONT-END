<template>
    <div style="padding-top:15px;" class="u-crud">
        <avue-crud ref="crud" :page.sync="page" v-model="form" :option="tableOption" :data="list" :search.sync="search"
            ...
            <template slot="cert" slot-scope="{row}">
                <div>
                    <dl class="dl">
                        <dt>基本信息</dt>
                        <dd><span>主题名称: </span> <span>{{ row.cert.subject_dn }}</span></dd>
                        <dd><span>签发者名称: </span><span>{{ row.cert.issuer.common_name }}</span></dd>
                        <dd><span>使用者备用名称: </span><span>{{ row.cert.extensions.subjectAltName }}</span></dd>
                        <dd><span>序列号: </span><span>{{ row.cert.serial_number }}</span></dd>
                        <dd><span>时间: </span> <span>{{ row.cert.validity.start }}至{{ row.cert.validity.end }}</span></dd>
                    </dl>
                </div>
                <div style="margin-top:10px">
                    <dl class="dl">
                        <dt>指 纹</dt>
                        <dd><span>SHA-256:</span> <span>{{ row.cert.fingerprint.sha256 }}</span></dd>
                        <dd><span>SHA-1:</span> <span>{{ row.cert.fingerprint.sha1 }}</span></dd>
                        <dd><span>MD5:</span> <span>{{ row.cert.fingerprint.md5 }}</span></dd>
                    </dl>
                </div>
            </template>
...
<script>
import { getPage,delObj} from '@/api/task/cert'
import { tableOption } from '@/crud/task/child/cert'
export default {
    name: 'MenuList',
    components: {},
    props:{
