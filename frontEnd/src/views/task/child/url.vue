    ...
    <div style="padding-top:15px;" class="u-crud">
        <avue-crud ref="crud" :page.sync="page" v-model="form" :option="tableOption" :data="list" :search.sync="search"
            ...
            <template slot="menuLeft">
                ...
                <el-button size="mini" type="primary" icon="el-icon-download" @click="handleExport">导出URL信息</el-button>
            </template>
        </avue-crud>
    </div>
</template>
...
<script>
import { getPage,delObj,exportObj} from '@/api/task/url'
import { tableOption } from '@/crud/task/child/url'
export default {
    name: 'MenuList',
    components: {},
    props:{
                ...
                exportObj(Object.assign(params,_this.info)).then(res => {
                    ...
                    if (window.navigator.msSaveBlob) {
                        try {
                            ...
                            window.navigator.msSaveBlob(blobObject, 'url.txt');
                        } catch (e) {
                            console.log(e);
                        }
                    } else {
                        ...
                        link.href = url
                        link.setAttribute('download', 'url.txt')
