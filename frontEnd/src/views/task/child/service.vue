    ...
    <div style="padding-top:15px;" class="u-crud">
        <avue-crud ref="crud" :page.sync="page" v-model="form" :option="tableOption" :data="list" :search.sync="search"
            ...
            <template slot="info" slot-scope="{row}">
                <ul>
                    <li v-for="item in row.service_info" :key="item.ip"><span>{{ item.ip }}</span></li>
                </ul>
            </template>
            <template slot="service_info.product" slot-scope="{row}">
                <span>{{row.service_info.product || '-'}}</span>
            </template>
...
<script>
import { getPage,delObj} from '@/api/task/service'
import { tableOption } from '@/crud/task/child/service'
export default {
    name: 'MenuList',
    components: {},
    props:{
